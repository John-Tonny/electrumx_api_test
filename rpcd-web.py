...
node_name = 'ElextrumX'
port = int(environ.get('RPC_PORT', 50002))
...
class RpcServer(BaseHTTPRequestHandler):
    ...
    async def send_request(self, request_self, method, params, rid):
        ...
        client=myssl.TcpConnection('/home/john/.electrum-dash')
        mySocket=client.get_socket()        
        #while(mySocket is None):
        #    mySocket=client.get_socket()        
                
        socketPipe=myssl.SocketPipe(mySocket)            
            
        #session=RPCSession(connection=mySocket)
#        async with Connector(RPCSession, 'localhost', client_port) as session:
